...
function Signup({ id }: SignupProps){
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      console.error("회원가입 또는 로그인 실패:", err.response?.data || err.message);
  ...
  return (
    <div id={id} className={styles.Signup_Form}>
      ...
      <form className={styles.Form} onSubmit={handleSubmit}>
        ...
        <Input id="email" className={styles.Input} name='email' type="email" placeholder="example@email.com" onChange={handleChange} value={values.email}/>
        <Label className={styles.Label} htmlFor="name">닉네임</Label>
        <Input id="name" className={styles.Input} name='name' type="name" placeholder="와인병" onChange={handleChange} value={values.name}/>
        <Label className={styles.Label} htmlFor="password">비밀번호</Label>
        <Input id="password" className={styles.Input} name='password' type="password" placeholder="8자 이상" value={values.password} onChange={handleChange}/>
        <Label className={styles.Label} htmlFor="passwordRepeat">비밀번호 확인</Label>
        <Input id="passwordRepeat" className={styles.Input} name='passwordRepeat' type="password" placeholder="비밀번호 확인" onChange={handleChange} value={values.passwordRepeat}/>
